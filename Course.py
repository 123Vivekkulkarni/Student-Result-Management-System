...
class CourseClass:
    def __init__(self,root):
        ...
        #-----Title-------
        title = Label(self.root, text="Manage Course Details", font=("goudy old style", 25, "bold"), bg="#033054", fg="white").place(x=10, y = 15, width=1180, height=35)
        ...
        lbl_Search_coursename = Label(self.root, text="Course Name:", font=("goudy old style",16,"bold"), bg="white").place(x=700, y=60)
        txt_Search_coursename = Entry(self.root,textvariable=self.search_var, font=("goudy old style",15,"bold"), bg="lightyellow").place(x=850, y=60, width=190)
...
#---------------------------------------------------------------------------
    ...
    def add(self):
        ...
        if self.var_course.get()=="":
            messagebox.showerror("Error","Course Name should be required",parent=self.root)
        elif course.isnumeric():
            messagebox.showerror("Error","Course Name should contain only letters",parent=self.root)
        elif charges.isalpha():
            messagebox.showerror("Error","Charges should be in Integer value",parent=self.root)
        else:
            try:
                cur.execute("select * from course where name=?",(self.var_course.get(),))
                row = cur.fetchone()
